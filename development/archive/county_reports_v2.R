...
# Probably divide this up into each section...
...
cohort_year <- strsplit(cohort_date, split="-")[[1]][1]
...
context_plot(ia_region, focus = ifelse(str_detect(focus_group_tx, pattern = fixed("Pierce")),
             ...
             title = paste("Investigations & Asessements: Region", region_cd),
             title_size = plot_title_size)
...
## ooh_safety----
## @knitr ooh_safety
names(ooh_safety_data)[1] <- ""
print.xtable(xtable(ooh_safety_data[ooh_safety_data[, 1] %nin% omit_ooh_counties, ],
...
ggplot(wb_context, aes(x = V1 / 100, y = county, color = context_highlight)) +
    ...
    labs(x = "Percent of Care-Days Spent in Kinship Care (2008-2012)",
         y = "",
         title = paste0("Kinship Care: Region ", region_cd)) +
