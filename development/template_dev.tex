...
\lhead{\color{pocDGreen} Thurston County Report}
...
\begin{tikzpicture}[x=1in, y=1in]
    ...
    \node[pocDBlue, below left = 6pt, align = right] at (\banX, \banY)
        {\textbf{Based on FamLink Data}\\\textbf{Extracted on February 06, 2015}};
    ...
    \node[right = 6pt, white] at (0, \banY - \stripeYpos - \stripeHeight + 16pt)
        {\fontspec{Archer}\Huge{Focus on Thurston County}};
    
    %% Below Stripe Text
    \node[below right = 6pt, pocDBlue, align = left, text width = \textwidth - 12pt]
        at (0, \banY - \stripeYpos - \stripeHeight) {
         This \emph{Focus on Thurston County} report is one of a series of automated county reports for Washington State. These quarterly reports mirror the Child Well-Being Data Portal by providing information on three major parts of the child welfare system: Investigations \& Assessments, Placement Prevention Services, and Out-of-Home Care. To access the Data Portal and the latest county reports, visit \href{http://www.partnersforourchildren.org/data-portal}{www.partnersforourchildren.org/data-portal}.} ;  
\end{tikzpicture}
...
\section*{Overview}
\vspace{-6pt}
The Department of Social and Health Services (DSHS) divides Washington State into three regions. Thurston County is in Region~3, along with Clallam, Clark, Cowlitz, Grays Harbor, Jefferson, Kitsap, Lewis, Mason, Pacific, Pierce, Skamania and Wahkiakum counties. The following report includes basic graphs and tables for Investigations \& Assessments, Placement Prevention Services and Out-of-Home Care in Thurston County}.

The Investigations \& Assessments section (page \pageref{p:ia}) and the Placement Prevention Services section (page \pageref{p:ihs}) provide household data on, respectively, the total number of investigations and assessments and in-home service cases since 2010, as well as comparative rates for investigations and assessments and in-home service cases for all Region~3 counties.
...
We hope this report serves as a valuable tool for  Thurston County.
...
The graph displays point-in-time (i.e., first day of the quarter) trends in investigations \& assessments cases for Thurston County.   %if another graph is added, update this.
\begin{knitrout}
...
The following display point-in-time (i.e., first day of the quarter) trends in Placement Prevention Services cases for Thurston County. % update if second graph is added
\nopagebreak[3]
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}
