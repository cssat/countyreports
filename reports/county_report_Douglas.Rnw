...
\section*{Overview}
\vspace{-6pt}
The Department of Social and Health Services (DSHS) divides Washington State into three regions. \Sexpr{display_county} County is in Region~\Sexpr{region_cd}, along with \Sexpr{comma_vector(region_counties_tx[-which(region_counties_tx == display_county)], oxford = FALSE)} counties. The following report includes basic graphs and tables for Investigations \& Assessments, In-Home Services and Out-of-Home Care in \Sexpr{display_county} County}.

The Investigations \& Assessments section (page \pageref{p:ia}) and the In-Home Services section (page \pageref{p:ihs}) provide household data on, respectively, the total number of investigations and assessments and in-home service cases since \Sexpr{year(ihs_start_date)}, as well as comparative rates for investigations and assessments and in-home service cases for all Region~\Sexpr{region_cd} DCFS (Division of Children \& Family Services) office(s)/office group(s) and for Washington State.
...
The following graph(s) display point-in-time (i.e., first day of the quarter) trends in investigations \& assessments cases for the DCFS office(s)/office group(s) in \Sexpr{display_county} County.  
<<ia_focus, cache=FALSE, echo=FALSE, fig.keep="high", fig.width=fig_width, fig.height=trend_fig_a * length(focus_office) + trend_fig_b, message = FALSE>>=
@
\end{minipage}
...
To give context to the \Sexpr{display_county} County offices' trend data, the following plot compares the rate of investigations \& assessments on \Sexpr{ia_context_date_string} for each Region~\Sexpr{region_cd} office/office group and for Washington State.  Rates, rather than counts (or total numbers), are used because they account for the population differences across Washington State counties.
\nopagebreak[3]
<<ia_context, cache=FALSE, echo=FALSE, fig.width=fig_width, fig.height=context_fig_a_2*length(region_groups) + context_fig_b>>=
@
...
The following graph(s) display point-in-time (i.e., first day of the quarter) trends in in-home services cases for the DCFS offices in \Sexpr{display_county} County. 
\nopagebreak[3]
<<ihs_focus, cache=FALSE, echo=FALSE, fig.keep="high", fig.width=fig_width, fig.height=trend_fig_a * length(focus_office) + trend_fig_b, message = FALSE>>=
@
\end{minipage}
...
To give context to the \Sexpr{display_county} County office(s') trend data, the following plot compares the rate of in-home services in quarter \Sexpr{quarter(as.POSIXlt(context_date))} of \Sexpr{year(as.POSIXlt(context_date))} for each Region~\Sexpr{region_cd} office and for Washington State. Rates, rather than a total number, are used because they account for the population differences across Washington State counties.
\nopagebreak[3]
<<ihs_context, cache=FALSE, echo=FALSE, fig.width=fig_width, fig.height=context_fig_a_2*length(region_groups) + context_fig_b>>=
@
...
%\nopagebreak[3]
<<ooh_context, cache=FALSE, echo=FALSE, fig.width=fig_width, fig.height=context_fig_a_1*length(region_counties) + context_fig_b>>=
...
\nopagebreak[3]
<<ooh_wb, cache=FALSE, echo=FALSE, fig.width=0.9*fig_width, fig.height=context_fig_a_1*length(region_counties) + context_fig_b>>=
@
\vspace{-18pt}
