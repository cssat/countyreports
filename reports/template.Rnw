...
\section*{Overview}
\vspace{-6pt}
The Department of Children, Youth and Families (DCYF) divides Washington State into three regions. \Sexpr{display_county} County is in Region~\Sexpr{region_cd}, along with \Sexpr{comma_vector(region_counties_tx[-which(region_counties_tx == display_county)], oxford = FALSE)} counties. 
