...
% \definecolor{pocLGreen}{HTML}{A2B69A} new green!
\definecolor{pocDBlue}{HTML}{244356}
% \definecolor{pocDBlue}{HTML}{3B6E8F} new blue!
...
\begin{tikzpicture}[x=1in, y=1in]
    ...
    \node[below right = 6pt, pocDBlue, align = left, text width = \textwidth - 12pt]
        at (0, \banY - \stripeYpos - \stripeHeight) {
         This \emph{Focus on \Sexpr{display_county} County} report is one of a series of automated county reports for Washington State. These quarterly reports mirror the Child Well-Being Data Portal by providing information on two major parts of the child welfare system: Investigations \& Assessments and Out-of-Home Care. To access the Data Portal and the latest county reports, visit \href{http://www.pocdata.org}{www.pocdata.org}.} ;  
\end{tikzpicture}
...
\section*{Overview}
\vspace{-6pt}
The Department of Social and Health Services (DSHS) divides Washington State into three regions. \Sexpr{display_county} County is in Region~\Sexpr{region_cd}, along with \Sexpr{comma_vector(region_counties_tx[-which(region_counties_tx == display_county)], oxford = FALSE)} counties. The following report includes basic graphs and tables for Investigations \& Assessments and Out-of-Home Care in \Sexpr{display_county} County}.

The Investigations \& Assessments section (page \pageref{p:ia}) provides household data on the total number of investigations and assessments cases since \Sexpr{year(ihs_start_date)}, as well as comparative rates for investigations and assessments cases for all Region~\Sexpr{region_cd} counties.
...
@
U.S. Census data is retrieved from the most recent State and County Quickfacts \newline
(\href{http://quickfacts.census.gov/qfd/states/53000.html}{http://quickfacts.census.gov/qfd/states/53000.html}).\\
